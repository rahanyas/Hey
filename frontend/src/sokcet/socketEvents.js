...
import { addMessageToState, deletMsgFromState } from "../features/messages/msgServices.js";
...
export const deleteMsg = (data) => {
    if(!socket) return;
    socket.emit('delete-msg', data)
}

export const registerSocketListeners = () => {
    if(!socket) return;

    // removing old listeners before listening again , prevent event duplication
    ...
    socket.on('msg-deleted', (msgId) => {
        store.dispatch(deletMsgFromState(msgId))
    })
}